...
public class MQProducerService {
	...
	// 建议正常规模项目统一用一个TOPIC
    private static final String topic = "kingsTopic";
    ...
    public void send(Order order) {
        rocketMQTemplate.convertAndSend(topic + ":tag1", order);
//        rocketMQTemplate.send(topic + ":tag1", MessageBuilder.withPayload(user).build()); // 等价于上面一行
    ...
    public void sendDelayMsg(Order order, int delayLevel) {
        rocketMQTemplate.syncSend(topic+"tag3", MessageBuilder.withPayload(JSON.toJSONString(order)).build(), messageTimeOut, delayLevel);
    }
